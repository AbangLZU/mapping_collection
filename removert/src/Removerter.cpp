...
{
    // int unused = system((std::string("exec rm -r ") + _path).c_str());
    // unused = system((std::string("mkdir -p ") + _path).c_str());
    fs::path m_path(_path);
    if (!fs::is_directory(m_path) || !fs::exists(m_path)) 
        fs::create_directories(m_path); // create src folder
} //fsmkdir
