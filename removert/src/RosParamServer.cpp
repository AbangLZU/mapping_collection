...
: nh(nh_super), ROSimg_transporter_(nh)
{
    ...
    fs::path pcd_dir(sequence_scan_dir_);
    for(auto& _entry : fs::directory_iterator(pcd_dir)) {
        sequence_scan_names_.emplace_back(_entry.path().filename().string());
        sequence_scan_paths_.emplace_back(_entry.path().string());
    }
